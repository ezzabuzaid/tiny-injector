...
> Calling `Add{Lifetime}` more than one time will result in throwing `ServiceExistException`, if your intentions
> to have register multiple implementations, check out `Append{lifetime}` instead.
