...
if the array type omitted the last registered implementation will be returned.
...
Another approache where you use service locator approache to get the requested instance
...
const scopedService3 = Injector.GetRequiredService(ScopedService, secondContext); // will return new instance
```
...
Injector.GetRequiredService({serviceType: Logger, multiple: true, context: /** In case of Scoped or Transient */});
```
